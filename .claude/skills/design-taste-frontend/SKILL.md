...
description: "Design and build NEW frontend UI from a blank slate so it does not look templated or AI-generated. Use when: adding a section, page, or component that does not exist yet, building a landing or marketing page, laying out a new blog or portfolio surface, or picking a design direction, type scale, or palette for new work. Triggers on phrasing like 'add a testimonials section', 'build a contact page', 'design a layout for X'. Does not audit or rework UI that already ships — escalates to redesign-existing-projects for that. Not for dashboards, data tables, or multi-step product UI."
---
